...
let firstLast = [];
firstLast = people.map((peope) => {
  const [last, first] = peope.split(", ");
  return `${first}  ${last}`;
});
...
// Complete the exercise in the space below:
commentById = comments.find((com) => com.id === 823423);
...
idx = comments.findIndex((com) => com.id === 123523);
// Check your work:
